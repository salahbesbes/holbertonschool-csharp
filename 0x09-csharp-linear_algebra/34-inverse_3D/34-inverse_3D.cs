...
internal class MatrixMath
{
    ...
    /// <summary> insert col at some position in the matrix  </summary>
    /// <param name="matrix"> origina matrix </param>
    /// <param name="pos"> index of the col to intsert </param>
    /// <param name="col"> col to insert </param>
    /// <returns> the matrix updated </returns>
    private static double[,] ReplaceColAtPosition(double[,] matrix, int pos, double[] col)
    {
    ...
    public static double[,] Inverse3D(double[,] matrix)
    {
        ...
        if (height == 3 && width == 3)
        {
            ...
            double originalDeterminant = Determinant(matrix);
            double[] row0 = new double[3] { 1, 0, 0 };
            double[] row1 = new double[3] { 0, 1, 0 };
            double[] row2 = new double[3] { 0, 0, 1 };
            double[,] newMatrix;
            double det;
            ...
            for (int i = 0; i < height; i++)
            {
                for (int j = 0; j < width; j++)
                {
                    ...
                    {
                        case 0:
                            newMatrix = ReplaceColAtPosition(matrix, i, row0);
                            det = Determinant(newMatrix);
                            res[i, j] = Math.Round(det / originalDeterminant, 2);
                            break;
                        case 1:
                            newMatrix = ReplaceColAtPosition(matrix, i, row1);
                            det = Determinant(newMatrix);
                            res[i, j] = Math.Round(det / originalDeterminant, 2);
                            break;
                        case 2:
                            newMatrix = ReplaceColAtPosition(matrix, i, row2);
                            det = Determinant(newMatrix);
                            res[i, j] = Math.Round(det / originalDeterminant, 2);
                            break;
                        default:
